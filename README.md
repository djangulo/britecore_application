...
You would notice that only the RiskType model's name is unique (the FieldType's is not), this was by design, since the frontend tightly couples the FieldType to the RiskType on creation. This prevents accidentally deleting some other RiskType's fields when deleting any RiskType instance.
...
For the frontend, I decided to use Vue.js with Vuex, as the state management of Vuex can greatly provides the interaction needed between the components. All API calls and form rendering happen through the Vue application.
...
For deployment, there were several steps that needed to cover:

- The PostgreSQL database, the API and the front-end would all end up in different locations
    - This was done so in order to leverage the convenience of Zappa (and the power of AWS)
    - This meant that it would have to happen sequentially, as each resource needed the addresses at leat one other resource
...
3. Bonus points if you also orchestrate the launch environment in AWS using CloudFormation.
    - Thanks to Boto3, Zappa and [](https://www.npmjs.com/package/vue-cli-plugin-s3-deploy) did not see the need of orchestrating the creation of say, an EC2 instance when the AWS-lambda deployment is serverless. If you would accept it, I would like to offer the [docker deployment](#deploy-aws-ec2) instead.
